...
__global__ void convolution_kernel(float * __restrict__ output, float * __restrict__ input, float * __restrict__ filter) {
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int sy = threadIdx.y;
    int sx = threadIdx.x;
    int g_idx = y * input_width + x; //Global index - Used twice
    int sx_bw = sx + border_width; //Used twice
    int sy_bw = sy + border_width; //Used twice


    //declare shared memory for this thread block
    __shared__ float sh_input[block_size_y + border_width][block_size_x + border_height];

    //Load elements that correspond to thread indices
    sh_input[sy][sx] = input[g_idx];

    //Load elements (Bottom, Right, Corner in that order) that dont directly correspond
    //to global indexes of this threadblock. This will generate more requests to global
    //memory but most of them will be coalesced (consecutive access pattern).
    sh_input[sy_bw][sx] = input[(y + border_width) * input_width + x];
    sh_input[sy][sx_bw] = input[g_idx + border_width];
    sh_input[sy_bw][sx_bw] = input[(y + border_width)* input_width + x + border_width];
    ...
    #pragma unroll
    for (int i=0; i < filter_height; i++) {
        //Reduce the index calculations for inner loop
        int sy_i = sy + i;
        int i_filter_width = i * filter_width;
        #pragma unroll
        for (int j=0; j < filter_width; j++) {
            sum += sh_input[sy_i][sx+j] * filter[i_filter_width+j];
        }
